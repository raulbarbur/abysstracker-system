...
export async function processSale(
  ...
  try {
    const paymentStatus: PaymentStatus =
      paymentMethod === "CHECKING_ACCOUNT" ? "PENDING" : "PAID";
    const paidAt = paymentStatus === "PAID" ? new Date() : null;

    if (paymentStatus === "PENDING" && !customerId) {
      return {
        error: "Para fiar (Cuenta Corriente) debés seleccionar un cliente.",
      };
    }
      ...
      async (tx) => {
        ...
        for (const item of cart) {
          if (item.type === "PRODUCT") {
            ...
            if (!variant)
              throw new Error(`Producto no encontrado: ${item.description}`);
            ...
            if (item.quantity <= 0 || isNaN(item.quantity)) {
              throw new Error(
                `Cantidad inválida para producto: ${item.description}`,
              );
            }
            ...
            let subtotal = 0;
            const uom = variant.product.unitOfMeasure;

            if (uom === UnitOfMeasure.GRAM) {
              const pricePerGram = effectivePrice / 1000;
              subtotal = round(pricePerGram * item.quantity);
            } else {
              subtotal = round(effectivePrice * item.quantity);
            }
            ...
            if (servicePrice < 0 || isNaN(servicePrice)) {
              throw new Error(
                `Precio de servicio inválido: ${item.description}`,
              );
            }
            if (item.quantity <= 0)
              throw new Error(`Cantidad inválida para servicio.`);

            const subtotal = round(servicePrice * item.quantity);
            totalReal = round(totalReal + subtotal);
      ...
      for (const item of sale.items) {
        if (item.variantId && item.variant) {
          ...
          if (item.settledQuantity > 0) {
            const amountOwedBack = round(
              Number(item.costAtSale) * item.settledQuantity,
            );
