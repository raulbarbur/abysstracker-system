...
export default function SettlementTicket({ data }: { data: SettlementData }) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(
        `Liquidacion_${data.owner.name}_${data.createdAt.toISOString().split("T")[0]}.pdf`,
      );
      addToast("PDF descargado correctamente", "success");
    } catch (e) {
      console.error(e);
      addToast("Error generando PDF. Intenta de nuevo.", "error");
    } finally {
  ...
  return (
    <div className="flex flex-col items-center gap-8 p-8 min-h-screen bg-background animate-in fade-in">
      <div className="flex gap-4 w-full max-w-2xl">
        <Link
          href={`/owners/balance`}
          className="px-5 py-3 bg-secondary hover:bg-accent text-foreground rounded-xl font-bold text-sm border border-border transition flex items-center gap-2"
        >
        ...
        <button
          ...
          className={cn(
            "flex-1 bg-primary hover:bg-primary/90 text-primary-foreground py-3 rounded-xl font-bold shadow-lg transition active:scale-95 flex items-center justify-center gap-2",
      ...
      >
        <div className="border-b-2 border-slate-900 pb-6 mb-8 flex justify-between items-start">
          <div className="flex flex-col gap-2">
            <Icon name="finance" className="h-8 w-8 text-slate-900" />
            <div>
        ...
        <div className="mb-8 p-4 bg-slate-50 rounded border border-slate-100 flex items-start gap-3">
          <Icon
            name="owners"
            className="h-5 w-5 text-slate-400 mt-1 shrink-0"
          />
          <div>
            ...
            <p className="text-slate-600">
              {data.owner.email || "Sin email registrado"}
            </p>
          </div>
        </div>

        <div className="mb-8">
          <h3 className="font-bold text-slate-700 border-b pb-2 mb-2 uppercase text-xs">
            Productos Vendidos
          </h3>
                ...
                data.items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="py-2 text-right">
                      $
                      {(
                        Number(item.costAtSale) * item.quantity
                      ).toLocaleString()}
                    </td>
                  </tr>
                ))
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {data.adjustments.map((adj) => (
                  <tr key={adj.id}>
                    ...
                    <td
                      className={`py-2 text-right font-bold ${Number(adj.amount) < 0 ? "text-red-600" : "text-slate-800"}`}
                    >
        ...
        <div className="mt-12 pt-8 border-t border-slate-200 text-center text-xs text-slate-400">
          <p>Comprobante generado electrónicamente por Sistema AbyssTracker.</p>
        </div>
      </div>
    </div>
  );
