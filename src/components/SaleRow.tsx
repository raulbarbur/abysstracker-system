...
import { cn, getPaymentMethodStyles, formatCurrency } from "@/lib/utils";
...
export default function SaleRow({ sale }: { sale: SaleData }) {
  ...
  const dateStr = createdAtDate.toLocaleDateString();
  ...
  const { badge, label } = getPaymentMethodStyles(sale.paymentMethod);
        ...
        <td className="hidden md:table-cell p-4">
          <span
            className={cn(
              ...
              isCancelled
                ? "line-through decoration-destructive opacity-50"
                : "text-foreground",
            )}
          >
            {formatCurrency(sale.total)}
