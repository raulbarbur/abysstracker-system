...
export default async function OwnerProfilePage({ params }: Props) {
  ...
  owner.products.forEach((p) => {
    p.variants.forEach((v) => {
      v.saleItems.forEach((item) => {
        ...
        if (pendingQty > 0) {
          let quantityFactor = pendingQty;
          if (p.unitOfMeasure === UnitOfMeasure.GRAM) {
            quantityFactor = pendingQty / 1000;
          }
  ...
  const debtFromAdj = owner.balanceAdjustments.reduce(
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 md:space-y-8 animate-in fade-in">
      ...
      <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                ...
                className="text-xs font-bold bg-card text-muted-foreground p-1.5 rounded border hover:text-foreground transition flex items-center justify-center"
              >
        ...
        <div className="flex flex-wrap gap-3">
          <InfoCard iconName="mail" label="Email" value={owner.email} />
          <InfoCard iconName="phone" label="Teléfono" value={owner.phone} />
        </div>
      </div>
        ...
        <div className="bg-card rounded-3xl shadow-sm border border-border overflow-hidden flex flex-col h-[500px]">
          <div className="p-5 border-b border-border bg-muted/30 flex justify-between items-center shrink-0">
            <h2 className="font-bold text-foreground flex items-center gap-2">
              ...
              En Stock ({activeInventory.length})
            </h2>
          ...
          <div className="overflow-y-auto divide-y divide-border custom-scrollbar flex-1">
            {activeInventory.length === 0 ? (
              ...
              activeInventory.map((item, idx) => (
                <div
                  key={idx}
                  className="p-4 flex justify-between items-center hover:bg-muted/30 transition"
                >
                  <div className="flex items-center gap-3">
                    {item.image ? (
                      <img
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Stock:{" "}
                        {item.unit === "GRAM"
                          ? `${(item.stock / 1000).toLocaleString("es-AR")} Kg`
                          : `${item.stock} u.`}
                      </p>
                    </div>
                  </div>
          ...
          <div className="overflow-y-auto divide-y divide-border custom-scrollbar flex-1">
            {owner.settlements.length === 0 ? (
              ...
              owner.settlements.map((settlement) => (
                <div
                  key={settlement.id}
                  className="p-4 flex justify-between items-center hover:bg-muted/30 transition group"
                >
                  <div>
                    <p className="font-bold text-foreground text-sm">
